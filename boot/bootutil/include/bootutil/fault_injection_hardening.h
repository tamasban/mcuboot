 ...
 *
 * DOUBLE_VARS makes critical variables into a tuple (x, x ^ msk). Then the
 * correctness of x can be checked by XORing the two tuple values together. This
 * also means that comparisons between fih_ints can be verified by doing
 * x == y && x_msk == y_msk.
 *
 * GLOBAL_FAIL makes all while(1) failure loops redirect to a global failure
 * loop. This loop has mitigations against loop escapes / unlooping. This also
 * means that any unlooping won't immediately continue executing the function
 * that was executing before the failure.
 *
 * CFI (Control Flow Integrity) creates a global counter that is incremented
 * before every FIH_CALL of vulnerable functions. On the function return the
 * counter is decremented, and after the return it is verified that the counter
 * has the same value as before this process. This can be used to verify that
 * the function has actually been called.
 *
 * DELAY causes random delays. This makes it hard to cause faults precisely.
 * DELAY requires an RNG. An mbedtls integration is provided in
...
#if defined(MCUBOOT_FIH_PROFILE_MAX)
...
#elif defined(MCUBOOT_FIH_PROFILE_MODERATE)
 ...
 */
#ifndef MCUBOOT_FIH_PROFILE_NONE
...
/* Label for interacting with FI testing tool. Can be parsed from the elf file
